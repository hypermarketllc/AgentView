/**
 * system-health-data-display-check-fixed.js
 * 
  ...
  try {
    ...
    if (healthChecksCount > 0) {
      ...
      const validChecksResult = await pool.query(`
        ...
        WHERE endpoint IS NOT NULL AND status IS NOT NULL
      `);
...
async function addHealthCheckRecords() {
  ...
  try {
    ...
    await addHealthCheck('/api/user-settings', 'User Settings', 'OK', 200);
    
    // Add health check for system health checks
    await addHealthCheck('/api/system-health-checks', 'System Health', 'OK', 200);
    
    // Add health check for settings
    await addHealthCheck('/api/settings', 'Settings', 'OK', 200);
    
    // Add health check for user accounts
    await addHealthCheck('/api/user-accs', 'User Accounts', 'OK', 200);
...
async function addHealthCheck(endpoint, category, status, statusCode) {
  const id = uuidv4();
  const responseTime = Math.floor(Math.random() * 100) + 50; // Random response time between 50-150ms
  
  await pool.query(`
    INSERT INTO system_health_checks
    (id, endpoint, category, status, response_time, status_code, created_at)
    VALUES ($1, $2, $3, $4, $5, $6, NOW())
  `, [id, endpoint, category, status, responseTime, statusCode]);
  
  console.log(`Added health check for ${category}: ${status}`);
}
