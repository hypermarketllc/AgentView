 ...
 */
async function logErrorToDB(errorType, message, details = {}, stackTrace = null) {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO system_errors 
       ...
       VALUES ($1, $2, $3, $4) 
       RETURNING *`,
      [errorType, message, details ? JSON.stringify(details) : null, stackTrace]
    );
    
    console.log(`Error logged to database: ${errorType} - ${message}`);
 ...
 */
async function getRecentErrors(limit = 100, includeResolved = false) {
  let client;
  try {
    client = await pool.connect();
 ...
 */
async function markErrorResolved(id, resolvedBy, notes = '') {
  let client;
  try {
    client = await pool.connect();
