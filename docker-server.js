...
import './docker-path-to-regexp-fix.mjs';
...
const pool = new Pool({
  ...
  port: parseInt(process.env.POSTGRES_PORT || '5432'), // Inside container, still use 5432
...
// Read the index.html file - try dist first, then fall back to public
let indexPath = path.join(__dirname, 'dist', 'index.html');
let indexHtml = '';

try {
  indexHtml = fs.readFileSync(indexPath, 'utf8');
  console.log('Successfully read index.html from dist directory');
} catch (err) {
  console.log('Error reading index.html from dist directory, trying public directory');
  try {
    indexPath = path.join(__dirname, 'public', 'index.html');
    indexHtml = fs.readFileSync(indexPath, 'utf8');
    console.log('Successfully read index.html from public directory');
  } catch (err) {
    console.error('Error reading index.html from public directory:', err);
...
// Update API paths in the public/index.html file
indexHtml = indexHtml.replace(/fetch\('\/api\//g, "fetch('/crm/api/");
...
app.use('/crm', express.static(path.join(__dirname, 'dist'), {
  setHeaders: (res, filePath) => {
    ...
    if (filePath.endsWith('.js') || filePath.includes('assets/index') && filePath.includes('.js')) {
      ...
      console.log(`Setting Content-Type for ${filePath}: text/html`);
    }
  }
}));
...
app.get('/crm/assets/*', (req, res, next) => {
  ...
  let filePath = path.join(__dirname, 'dist', assetPath);
...
app.post('/crm/api/auth/login', async (req, res) => {
  try {
    ...
    // Special case for admin@americancoveragecenter.com
    if (email === 'admin@americancoveragecenter.com') {
      ...
      console.log('Admin user found:', user.rows.length > 0 ? 'yes' : 'no');
      ...
      if (user.rows.length === 0) {
        ...
        if (positionResult.rows.length === 0) {
          return res.status(500).json({ error: 'Admin position not found' });
        }
