/**
 * apply-missing-tables-fixed.js
 * 
 * This script applies the SQL in create-missing-tables-fixed.sql to create
...
const sqlFilePath = './create-missing-tables-fixed.sql';
  ...
  for (const table of tables) {
    try {
      ...
      if (exists) {
        ...
        console.log(`   - Columns: ${columnsResult.rows.map(row => `${row.column_name} (${row.data_type})`).join(', ')}`);
      } else {
