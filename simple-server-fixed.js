...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith('/crm/api/')) {
    ...
    if (pathname === '/crm/api/deals' && req.method === 'POST') {
      try {
        const authHeader = req.headers['authorization'];
        const token = authHeader && authHeader.split(' ')[1];
        ...
        const body = await parseBody(req);
        const { carrierId, productId, clientName, annualPremium, appNumber, clientPhone, effectiveDate, fromReferral } = body;
        ...
        const result = await pool.query(
          `INSERT INTO deals 
           ...
           RETURNING *`,
          [dealId, auth.user.id, carrierId, productId, clientName, annualPremium, appNumber, clientPhone, effectiveDate, fromReferral, 'Submitted']
        );
