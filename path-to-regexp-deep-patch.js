// Create this as path-to-regexp-fix.js
const fs = require('fs');
...
let patchedCode = originalCode.replace(
  /if \(!value\) {\s*throw new TypeError\(`Missing parameter name at \${i}: \${DEBUG_URL}`\);\s*}/,
  `if (!value) {
...
console.log('✅ Successfully patched path-to-regexp to handle missing parameter names');